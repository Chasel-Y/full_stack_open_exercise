describe('Blog_list app', function() {
  ...
  describe('When logged in', function() {
    ...
    it('A blog can be created', function() {
      ...
      cy.get('.inform').should('contain', 'a new blog test blog1 added by test author1')
      cy.get('.inform').should('have.css', 'color', 'rgb(0, 128, 0)')
      cy.contains('test blog1 - test author1')
    })
    describe('Some blogs exist', function() {
      beforeEach(function() {
        cy.createBlog({ title: 'test blog1', author: 'test author1', url: 'test url1' })
      ...
      it('A blog can be liked', function() {
        ...
        cy.contains('test blog1 - test author1').parent().parent().contains('like').click()
        cy.get('.inform').should('contain', 'test blog1 get a like from chasel')
        cy.get('.inform').should('have.css', 'color', 'rgb(0, 128, 0)')
        cy.contains('likes 1')
      })
      it('the user who created a blog can delete it', function() {
        cy.contains('test blog1 - test author1').contains('view').click()
        cy.contains('test blog1 - test author1').parent().parent().contains('remove').click()
      ...
      it('only the creator can see the delete button of a blog', function() {
        ...
        cy.contains('test blog1 - test author1').contains('view').click()
        cy.contains('test blog1 - test author1').parent().parent().should('not.contain', 'remove')
      })
      it('blogs are ordered according to likes', function() {
        cy.contains('test blog2 - test author2').contains('view').click()
        cy.contains('test blog2 - test author2').parent().parent().contains('like').click()
        cy.contains('test blog3 - test author3').contains('view').click()
        cy.contains('test blog3 - test author3').parent().parent().contains('like').click()
        cy.contains('test blog3 - test author3').parent().parent().contains('like').click()
